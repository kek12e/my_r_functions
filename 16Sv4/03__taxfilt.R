...
## ---- filtering taxa ------------------------------------------------------------------- ##
ps = readRDS(file.path(work.dir,"decontam","ps.p.decontam.prev0.5.RDS"))
ps.nonc = prune_samples(!(sample_names(ps) %>% grepl(pattern = "NC|NFW")), 
                        ps)
...
seqtab.p = as.data.frame(readRDS("./rds/seqtab.p.RDS")); colnames(seqtab.p) = seq(1,ncol(seqtab.p))
seqtab.nochim.p = as.data.frame(readRDS("./rds/seqtab.p.nochim.RDS")); colnames(seqtab.nochim.p) = seq(1,ncol(seqtab.nochim.p))
...
asv.track$kng.bact = rowSums(otu_table(
								ps %>% 
								subset_taxa(Kingdom == "Bacteria")
							) != 0 )
asv.track$phy.nona = rowSums(otu_table( 
								ps %>% 
								subset_taxa(Kingdom == "Bacteria") %>% 
								subset_taxa(Phylum != "unclass. Bacteria")
							) != 0 )
asv.track$ord.nchl = rowSums(otu_table( 
								ps %>% 
								subset_taxa(Kingdom == "Bacteria") %>% 
								subset_taxa(Phylum != "unclass. Bacteria") %>%
								subset_taxa(Order != "Chloroplast")
							) != 0 )
asv.track$fam.nmit = rowSums(otu_table( 
								ps %>% 
								subset_taxa(Kingdom == "Bacteria") %>% 
								subset_taxa(Phylum != "unclass. Bacteria") %>%
								subset_taxa(Order != "Chloroplast") %>%
...
write.csv(as.data.frame(asv.track), "./csv/asv.track.csv",quote=F,row.names=T)
